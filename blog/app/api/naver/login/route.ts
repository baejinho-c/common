...
export async function GET() {
  try {
    ...
    const redirectUri = encodeURIComponent(`${process.env.NEXT_PUBLIC_BASE_URL}/api/naver/login/callback`)
    ...
    return NextResponse.json({
      success: true,
      loginUrl: naverLoginUrl,
      state,
    })
  } catch (error) {
    console.error("네이버 로그인 URL 생성 오류:", error)
    return NextResponse.json({ error: "네이버 로그인 URL 생성에 실패했습니다." }, { status: 500 })
  }
}
