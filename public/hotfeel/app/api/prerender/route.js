"use strict";(()=>{var e={};e.id=403,e.ids=[403],e.modules={399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},1761:(e,t,r)=>{r.r(t),r.d(t,{originalPathname:()=>v,patchFetch:()=>f,requestAsyncStorage:()=>h,routeModule:()=>u,serverHooks:()=>w,staticGenerationAsyncStorage:()=>y});var i={};r.r(i),r.d(i,{GET:()=>d});var o=r(3278),a=r(5002),n=r(4877),s=r(1309),l=r(4265);let p=[{id:"1",title:"눈물의 여왕",category:"드라마",poster:"/placeholder.svg?height=200&width=150&text=눈물의여왕",rating:4.8,year:2024,genre:["로맨스","멜로"],description:"재벌 3세와 슈퍼마켓 딸의 결혼 생활을 그린 로맨틱 코미디",status:"completed",episodes:16},{id:"2",title:"외모지상주의",category:"웹툰",poster:"/placeholder.svg?height=200&width=150&text=외모지상주의",rating:4.5,year:2018,genre:["드라마","학원"],description:"외모 때문에 괴롭힘을 당하던 소년의 이야기",status:"ongoing"},{id:"3",title:"기생충",category:"영화",poster:"/placeholder.svg?height=200&width=150&text=기생충",rating:4.9,year:2019,genre:["스릴러","드라마"],description:"계급 사회의 모순을 그린 봉준호 감독의 작품",duration:"132분"},{id:"4",title:"무한도전",category:"예능",poster:"/placeholder.svg?height=200&width=150&text=무한도전",rating:4.7,year:2006,genre:["버라이어티","코미디"],description:"대한민국 최고의 예능 프로그램",status:"completed"},{id:"5",title:"귀멸의 칼날",category:"애니",poster:"/placeholder.svg?height=200&width=150&text=귀멸의칼날",rating:4.6,year:2019,genre:["액션","판타지"],description:"도깨비가 된 여동생을 구하기 위한 소년의 모험",status:"ongoing",episodes:44},{id:"6",title:"미드나잇 라이브러리",category:"도서",poster:"/placeholder.svg?height=200&width=150&text=미드나잇라이브러리",rating:4.4,year:2020,genre:["판타지","철학"],description:"삶과 죽음 사이에서 만나는 무한한 가능성의 도서관"}];async function c(e){return new Promise(t=>{setTimeout(()=>{let r=[...p];e?.query&&(r=r.filter(t=>t.title.toLowerCase().includes(e.query.toLowerCase())||t.description.toLowerCase().includes(e.query.toLowerCase())||t.genre.some(t=>t.toLowerCase().includes(e.query.toLowerCase())))),e?.category&&(r=r.filter(t=>t.category.toLowerCase()===e.category.toLowerCase())),e?.genre&&e.genre.length>0&&(r=r.filter(t=>e.genre.some(e=>t.genre.some(t=>t.toLowerCase().includes(e.toLowerCase()))))),e?.year&&(r=r.filter(t=>t.year===e.year)),e?.rating&&(r=r.filter(t=>t.rating>=e.rating)),e?.status&&(r=r.filter(t=>t.status===e.status)),e?.limit&&(r=r.slice(e.offset||0,(e.offset||0)+e.limit)),t(r)},100)})}async function d(e){let t=e.headers.get("user-agent")||"",r=new URL(e.url).searchParams.get("path")||"/";if(!/googlebot|bingbot|slurp|duckduckbot|baiduspider|yandexbot|naverbot|facebookexternalhit|twitterbot|linkedinbot/i.test(t))return s.NextResponse.redirect(new URL(r,e.url));try{let e=await g(r);return new s.NextResponse(e,{headers:{"Content-Type":"text/html; charset=utf-8","Cache-Control":"public, max-age=3600, s-maxage=3600","X-Prerendered":"true"}})}catch(e){return console.error("Prerender error:",e),new s.NextResponse("Prerender failed",{status:500})}}async function g(e){let t=process.env.NEXT_PUBLIC_BASE_URL||"https://yojeum-nabwa.vercel.app",r="요즘 나봐 - 한국 콘텐츠 감상 SNS",i="드라마, 영화, 웹툰, 책 등 한국 콘텐츠에 대한 생생한 감상과 리뷰를 공유하는 소셜 플랫폼",o="",a="";if(e.startsWith("/post/")){let n=e.split("/")[2],s=(await (0,l.Bd)()).find(e=>e.id===n);s&&(r=`${s.content?.title||"콘텐츠"} - ${s.author.name}의 감상 | 요즘 나봐`,i=(s.review?.styled||s.review?.original||"").substring(0,160),o=function(e){let t=m(e.content?.type||"unknown"),r=e.review?.styled||e.review?.original||"";return`
    <article>
  ...
  `)}else if(e.startsWith("/content/")){let n=decodeURIComponent(e.split("/")[2]),s=(await c()).find(e=>e.title===n);s&&(r=`${s.title} 감상 모음 | 요즘 나봐`,i=`${s.title}에 대한 다양한 사용자들의 감상과 리뷰를 확인해보세요.`,o=`
    <article>
      <h1>${s.title}</h1>
      <div class="post-meta">
