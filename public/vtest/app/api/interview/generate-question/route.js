...
- Respond in Korean
- Keep total response under 150 words`}let{text:h}=await (0,c.Df)({model:t("gpt-4o-mini"),prompt:v,temperature:.7,maxTokens:300});return i.NextResponse.json({question:h.trim()})}let v=u[r]||u.general,h=d[n]||d.basic,f=o.map(e=>`${"interviewer"===e.type?"Interviewer":"Candidate"}: ${e.content}`).join("\n"),y="";if(1===a)y=`You are an AI interviewer conducting a mock interview. Your persona is: ${g.characteristics}.
